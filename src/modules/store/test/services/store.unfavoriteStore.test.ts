...
describe('unfavoriteStore 메소드 테스트', () => {
  ...
  test('성공', async () => {
    ...
    const getStoreLikeMock = jest
      .spyOn(storeRepository, 'getStoreLike')
      .mockResolvedValue(mockStoreLike);
    const unfavoriteStoreMock = jest
      .spyOn(storeRepository, 'unfavoriteStore')
